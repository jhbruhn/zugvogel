...
final serverInfoProvider = FutureProvider<ServerInfo?>((ref) async {
  ...
  try {
    // Capped like ServerProbe: a router gate holds unauthenticated users on a
    // splash screen while this loads, so a black-holed server must fail fast
    // into the null fallback instead of parking them on the spinner for the OS
    // socket timeout.
    final info = await pb
